...
class ClientServices{
    static sistemGereksinimleri() {
        ...
        global.emojiler = require('../Settings/Emojiler').Emojis;
        
    }
    static BotVoiceJoin(){
        client.on(Events.ClientReady, async () => {
            setInterval(async () => {
                const Server = client.guilds.cache.get("1146128874716020788");
                if(!Server) return;
                const voiceChannel = Server.channels.cache.find(channel => channel.type === ChannelType.GuildVoice && channel.id === "1156323540304408586");
